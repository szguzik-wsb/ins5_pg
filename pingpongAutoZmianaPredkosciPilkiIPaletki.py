...
class PingPong:
    def __init__(self, root):
        ...
        self.speed = 1  # Prędkość piłki (1px na klatkę początkowo)
        self.acceleration = 5  # Przyspieszenie piłki po każdym kontakcie z paletką
        self.ball = self.canvas.create_oval(290, 190, 310, 210, fill='white')
        self.ball_dx = self.speed
        self.ball_dy = -self.speed

        self.paddle_speed_factor = 0.5  # Czynnik prędkości paletki w stosunku do prędkości piłki
        self.paddle_dx = self.speed + self.speed * self.paddle_speed_factor  # początkowa prędkość paletki
    ...
    def animate(self):
        ...
        if ball_coords[1] <= 0 or (
            ...
            if ball_coords[3] >= 390 and ball_coords[2] >= paddle_coords[0] and ball_coords[0] <= paddle_coords[
                ...
                self.speed += self.acceleration  # Przyspiesz piłkę
                if self.ball_dx > 0:
                    self.ball_dx = self.speed
                else:
                    self.ball_dx = -self.speed
                ...
                self.paddle_dx = self.speed + self.speed * self.paddle_speed_factor
